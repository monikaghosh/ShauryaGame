...
if(keyWentDown("s")&&gamestate==="play"&& bulletcount>0){
  ...
  if(abs(player.player.x-enemy1.enemy.x)<150 && abs(player.player.y-enemy1.enemy.y)<150){
   createenemybullet(); 
  }
  if(abs(player.player.x-enemy2.enemy.x)<150 && abs(player.player.y-enemy2.enemy.y)<150){
    createenemybullet2(); 
   }
